...
export function removeEntriesFromTextPrompt(prompt, disabledEntries) {
    ...
    for (const entry of disabledEntries) {
        const patched = removeEntryContentOnce(result, entry);
        result = patched.text;
    }
...
function removeEntryContentOnce(text, entry) {
    ...
    const lineEndingPatchedContent = content.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    const lineEndingPatchedSource = source.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    const normalizedIndex = lineEndingPatchedSource.indexOf(lineEndingPatchedContent);

    if (normalizedIndex !== -1) {
        return {
            text: lineEndingPatchedSource.slice(0, normalizedIndex)
                + lineEndingPatchedSource.slice(normalizedIndex + lineEndingPatchedContent.length),
            removed: true,
        };
    }
