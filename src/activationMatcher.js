...
function getEntryMatchType(entry, promptText, normalizedPrompt) {
    ...
    if (promptText.includes(entry.content)) {
        return 'exact';
    }

    const normalizedContent = normalizeForMatching(entry.content);

    if (normalizedContent && normalizedPrompt.includes(normalizedContent)) {
        return 'normalized';
    }
    ...
    if (compactContent.length >= 80 && compactPrompt.includes(compactContent)) {
        return 'compact';
    }
