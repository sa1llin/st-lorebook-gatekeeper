...
function createEntryElement(entry, state, template, onToggle) {
    ...
    editButton.className = `lbg-icon-button lbg-direct-edit-button ${edited ? 'is-edited' : ''}`;
    editButton.textContent = '✎';
    editButton.title = 'Edit for this prompt';
...
function getSavedEditDialogHeight() {
    const minimum = Math.max(620, Math.round(window.innerHeight * 0.86));
    const maximum = Math.max(minimum, Math.round(window.innerHeight * 0.96));
...
function installEditDialogResize(handle, dialog) {
    ...
    const onMove = (event) => {
        ...
        const minimum = Math.max(560, Math.round(window.innerHeight * 0.78));
        const maximum = Math.max(minimum, Math.round(window.innerHeight * 0.96));
